...
def main(input_path, model_path, scale, outscale):
    ...
    parser.add_argument(
        "--input", type=str, default="inputs", help="Input image or folder"
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    parser.add_argument(
        "--netscale", type=int, default=4, help="Upsample scale factor of the network"
    )
    parser.add_argument(
        "--outscale",
        type=float,
    ...
    parser.add_argument(
        "--tile", type=int, default=800, help="Tile size, 0 for no tile during testing"
    )
    ...
    parser.add_argument(
        "--half", action="store_true", help="Use half precision during inference"
    )
    parser.add_argument(
        "--alpha_upsampler",
        type=str,
...
def supre_resolution(imageurl, scale_type):
    ...
    resp = (
        "https://magicstore.styldod.com/REimagine-images/upscaled/" + up_name + ".jpg"
    )
